...
@implementation Velvet3PreviewController

- (NSMutableArray*)visibleSpecifiersFromPlist:(NSString*)plist {
	Velvet3PrefsManager *manager = [NSClassFromString(@"Velvet3PrefsManager") sharedInstance];
...
-(void)viewDidLayoutSubviews {
	...
	self.preview = [[Velvet3PreviewView alloc] initWithFrame:previewFrame notificationWidth:notificationWidth identifier:self.identifier];

	if ([self.navigationController.previousViewController isKindOfClass:NSClassFromString(@"Velvet3SettingsController")]) {
		// Use the same icon that our parent displayed
		[self.preview updateAppIconWithIdentifier:((Velvet3SettingsController *)self.navigationController.previousViewController).preview.currentIconIdentifier];
	}
