...
@implementation Velvet3Colorizer

- (instancetype)initWithIdentifier:(NSString *)identifier {
    Velvet3Colorizer *colorizer = [super init];

    self.identifier = identifier;
    self.manager = [Velvet3PrefsManager sharedInstance];
...
- (void)colorTitle:(UILabel*)title {
    // Handed a private ivar: only a label can take a text colour. Anything else
    // (or nil) and we quietly do nothing rather than raise on an unknown selector.
